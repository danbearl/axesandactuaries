...
import { PROPERTY_CONFIG, UPGRADE_COSTS } from '../services/propertyCatalog.js';
  ...
  const [, updatedProperty] = await prisma.$transaction([
    ...
    prisma.property.update({
      ...
      data: {
        ...
        // Re-synced to the current catalog value on every upgrade, not just carried over —
        // otherwise a property built before a bonus-schema change stays stuck on its
        // stale bonus JSON forever, even as it levels up (see propertyCatalog.ts for the
        // incident this caused: a pre-redesign Training Hall reached level 3 showing a
        // +600% power bonus instead of +30%, because "upgrade" never touched `bonus`).
        bonus:                config.bonus,
      },
    }),
    prisma.transaction.create({
