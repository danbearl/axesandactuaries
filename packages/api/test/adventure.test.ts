...
describe('resolveAdventure', () => {
  ...
  it('grants a role-property XP bonus to priest-vocation adventurers with a Sanctuary', async () => {
    vi.spyOn(Math, 'random').mockImplementation(() => 0.99); // never injured

    const player = await createPlayer({ gold: 500 });
    await prisma.property.create({
      data: { playerId: player.id, type: 'sanctuary', level: 2, maintenanceCostDaily: 24, bonus: { xpBonusPerLevel: 0.1, loyaltyRecoveryBonus: 1 } },
    });
    const adventurer = await createAdventurer({
      employerId: player.id, status: 'on_adventure', powerRating: 50, experience: 0, level: 1,
      vocation: 'Mender', // priest role — matches Sanctuary
    });
    const contract = await createContract({
      requiredPower: 50, rewardGold: 300, reputationReward: 3,
      penaltyGold: 90, penaltyReputation: 1, status: 'in_progress',
    ...
    // base xp = 300 * 0.1 / 1 = 30; ambition 3 (default) -> x1.1; Sanctuary level 2 -> +20% -> x1.2
    const updatedPriest = await prisma.adventurer.findUniqueOrThrow({ where: { id: adventurer.id } });
    expect(updatedPriest.experience).toBe(Math.floor(30 * DEFAULT_AMBITION_MULTIPLIER * 1.2));
  });
